...
const GameManager: React.FC = () => {
  ...
  // Fonction de démarrage/reinitialisation de la partie
  const startGame = () => {
    setScore(0);
    setErrors(0);
    setArrows([]);
  ...
  // Mise à jour régulière de la position des flèches
  useEffect(() => {
  ...
  // Apparition des flèches à intervalle régulier
  useEffect(() => {
    if (!gameStarted || gameOver) return;
    const interval = setInterval(() => {
      spawnArrow();
  ...
  // Retirer les flèches qui ont dépassé la zone de jeu et compter les erreurs pour les flèches manquées
  useEffect(() => {
    if (!gameStarted || gameOver) return;
